...
function getWindowState(win) {
  ...
  return {
    ...
    isNativeFullScreen: isPlatformFullscreen(win),
    isHtmlFullScreen: htmlFullscreenActive,
    isWindowFullScreen: windowFullscreenActive,
    isFullScreen: isPlatformFullscreen(win) || htmlFullscreenActive || windowFullscreenActive,
...
// ponytail: macOS fullscreen uses manual bounds+alwaysOnTop because both
// setFullScreen (silently fails on transparent frameless window) and
// setSimpleFullScreen (triggers compositor mode switch that chokes on
// transparent surfaces, ~3 fps) are broken. Manual mode stays in normal
// rendering pipeline — no Space transition, no compositor mode switch.
// Upgrade: if macOS ever optimizes fullscreen compositing for transparent
// windows, switch back to setFullScreen.
function isPlatformFullscreen(win) {
  if (!win || win.isDestroyed()) return false;
  return IS_MAC ? windowFullscreenActive : win.isFullScreen();
}

function setPlatformFullscreen(win, flag) {
  if (!IS_MAC) { win.setFullScreen(flag); return; }
  if (flag) {
    var display = screen.getDisplayMatching(win.getBounds());
    win._savedFullscreenBounds = win.getBounds();
    win.maximize();
    ...
    if (win._savedFullscreenBounds) {
      win.setBounds(win._savedFullscreenBounds);
      delete win._savedFullscreenBounds;
    }
  }
}
...
function exitFullscreenToWindow(win) {
  ...
  if (IS_MAC) {
    ...
    setTimeout(function () { applyWindowedBounds(win); }, 50);
    return;
  }
  ...
  win.once('leave-full-screen', function () { setTimeout(applyOnce, 50); });
...
async function createWindow() {
  ...
  mainWindow = new BrowserWindow({
    ...
    transparent: !IS_MAC,
    backgroundColor: IS_MAC ? '#06080A' : '#00000000',
    hasShadow: true,
    autoHideMenuBar: true,
    title: APP_NAME,
    icon: APP_ICON_PATH,
  ...
  mainWindow.webContents.on('before-input-event', (event, input) => {
    if (input.type === 'keyDown' && (input.key === 'Escape' || input.code === 'Escape') && isPlatformFullscreen(mainWindow)) {
  ...
  mainWindow.on('unmaximize', () => {
    if (IS_MAC && windowFullscreenActive) {
      exitFullscreenToWindow(mainWindow);
      return;
    }
    sendWindowState(mainWindow);
