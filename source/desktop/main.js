...
function registerMacOSMediaKeys() {
  if (!IS_MAC) return;
  var defaults = [
    { action: 'togglePlay', key: 'MediaPlayPause' },
    { action: 'nextTrack', key: 'MediaNextTrack' },
    { action: 'prevTrack', key: 'MediaPreviousTrack' },
  ];
  for (var i = 0; i < defaults.length; i++) {
    var d = defaults[i];
    try { globalShortcut.register(d.key, function (action) { return function () { sendGlobalHotkeyAction(action); }; }(d.action)); } catch (e) {}
  }
}
...
function createDesktopLyricsWindow(payload = {}) {
  ...
  if (IS_MAC && !(payload && Object.prototype.hasOwnProperty.call(payload, 'clickThrough'))) {
    desktopLyricsState.clickThrough = false;
  }
